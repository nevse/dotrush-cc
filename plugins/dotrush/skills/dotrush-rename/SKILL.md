...
- `<path> is outside the workspace <root> (or under bin/obj); apply with --outside-workspace to edit it` (or `<paths> are outside the workspace <root> (or under bin/obj); apply with --outside-workspace to edit them`) — nothing was written. Ask the user whether those files may be edited; add `--outside-workspace` only if they agree, otherwise cancel.
